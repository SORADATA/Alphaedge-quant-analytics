...
from src.utils.math_utils import _rolling_maxdrawdown, _rolling_sortino, _safe_div
...
def _add_momentum_factors(df: pd.DataFrame, g) -> pd.DataFrame:
    ...
    df["mom_12_1"] = pct_12.div(pct_1.replace(0, np.nan)).replace(
        [np.inf, -np.inf], np.nan
    )
    df["mom_6_1"] = pct_6.div(pct_1.replace(0, np.nan)).replace(
        [np.inf, -np.inf], np.nan
    )
...
def _add_volatility_factors(df: pd.DataFrame, g) -> pd.DataFrame:
    df["realized_vol_3m"] = g["return_1m"].transform(
        lambda x: x.rolling(3, min_periods=2).std() * np.sqrt(12)
    )
    df["realized_vol_12m"] = g["return_1m"].transform(
        lambda x: x.rolling(12, min_periods=6).std() * np.sqrt(12)
    )
    df["vol_ratio"] = (
        df["realized_vol_3m"]
        .div(df["realized_vol_12m"])
        .replace([np.inf, -np.inf], np.nan)
    )
    excess = (
        df["return_1m"] - df["Mkt-RF"].fillna(0)
...
def _add_tail_risk_factors(df: pd.DataFrame, g) -> pd.DataFrame:
    df["return_skew_6m"] = g["return_1m"].transform(
        lambda x: x.rolling(6, min_periods=3).skew()
    )
    df["return_kurt_6m"] = g["return_1m"].transform(
        lambda x: x.rolling(6, min_periods=3).kurt()
    )
    df["hist_var_5pct"] = g["return_1m"].transform(
        lambda x: x.rolling(12, min_periods=6).quantile(0.05)
    )
...
def _add_technical_enrichment(df: pd.DataFrame, g) -> pd.DataFrame:
    if "rsi" in df.columns:
        df["rsi_divergence"] = g["adj close"].transform(lambda x: x.pct_change(3)) - g[
            "rsi"
...
def add_rank_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for feat in features_to_rank:
        if feat in df.columns:
            df[f"{feat}_rank"] = df.groupby(level="date")[feat].transform(
...
def compute_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker in df.index.get_level_values(1).unique():
        ...
        if len(close) > MIN_HISTORY_TA:
            df.loc[idx, "rsi"] = (
                RSIIndicator(close=close, window=RSI_WINDOW).rsi().values
            )
            bb = BollingerBands(
                close=np.log1p(close), window=BB_WINDOW, window_dev=BB_STD
            )
            df.loc[idx, "bb_low"], df.loc[idx, "bb_mid"], df.loc[idx, "bb_high"] = (
                bb.bollinger_lband().values,
                bb.bollinger_mavg().values,
                bb.bollinger_hband().values,
            )
            df.loc[idx, "bb_position"] = (np.log1p(close) - bb.bollinger_lband()) / (
    ...
    df["euro_volume"] = (
        (df["adj close"] * df["volume"]) / 1e6 if "volume" in df.columns else 0.0
    )
    return df
...
def get_fama_french_betas(data: pd.DataFrame, ff_region: str) -> pd.DataFrame:
    ...
    try:
        with warnings.catch_warnings():
            ...
            factor_data = web.DataReader(ff_region, "famafrench", start="2010")[0].drop(
                "RF", axis=1
            )
        ...
        for ticker in data.index.get_level_values(1).unique():
            ...
            params = (
                RollingOLS(
                    y, sm.add_constant(X[FAMA_FRENCH_FACTORS]), window=MIN_HISTORY_FF
                )
                .fit()
        ...
        if betas_list:
            ...
            data[FAMA_FRENCH_FACTORS] = data.groupby(level="ticker", group_keys=False)[
                FAMA_FRENCH_FACTORS
