...
import numpy as np
import pandas as pd
from pypfopt import EfficientCVaR, black_litterman, expected_returns, risk_models
...
def get_optimal_weights(
    ...
    probas_subset: pd.Series | None = None,
    ...
    try:
        ...
        prior = expected_returns.mean_historical_return(
            prices_df, frequency=TRADING_DAYS_YEAR
        )
        ...
        if probas_subset is not None and not probas_subset.empty:
            ...
            if len(valid_tickers) > 0:
                ...
                q_views = np.array(
                    [(probas_subset[t] - 0.5) * 0.5 for t in valid_tickers]
                )
                ...
                bl = black_litterman.BlackLittermanModel(
                    cov,
                    pi=prior,
                    absolute_views=dict(zip(valid_tickers, q_views)),
                    P=p_matrix,
                    Q=q_views,
                    omega=omega,
                )
                ...
                ef = EfficientCVaR(
                    ret_bl,
                    prices_df[valid_tickers].pct_change().dropna(),
                    beta=0.95,
...
def _build_price_matrix(
    df_daily: pd.DataFrame, ffill_limit: int = MAX_PRICE_FFILL_DAYS
) -> pd.DataFrame:
...
def _build_daily_snapshot(
    df_daily: pd.DataFrame, market_config: dict
...
def _filter_warmup_period(
    df_monthly: pd.DataFrame, min_history: int = MIN_FEATURE_HISTORY
) -> pd.Series:
...
def _simulate_period(
    ...
    turnover = (
        sum(
            abs(allocation.get(t, 0.0) - drifted_allocation.get(t, 0.0))
            for t in set(allocation) | set(drifted_allocation)
        )
        / 2.0
    )
    ...
    rets = (
        daily_returns.reindex(index=trading_days, columns=tickers)
        .fillna(0.0)
        .to_numpy()
    )
    growth = np.cumprod(1.0 + rets, axis=0)

    strategy_values = (portfolio_value * weights[np.newaxis, :] * growth).sum(
        axis=1
    ) + (portfolio_value * (1 - sum(weights)))

    # Application des frais de gestion au prorata journalier
    fee_daily_factor = 1.0 - MANAGEMENT_FEE_ANNUAL / 252
    strategy_values = strategy_values * np.cumprod(
        [fee_daily_factor] * len(strategy_values)
    )
    ...
    new_drifted = dict(
        zip(tickers, (portfolio_value * weights * growth[-1, :] / final_total))
    )
    return (
        pd.DataFrame(
            {
        ...
        final_total,
        float(bench_values[-1]),
        new_drifted,
    )


def backtest_strategy_with_rebalancing(
    df_daily, df_monthly, model, benchmark_ticker, market_config
):
    valid_history_mask = _filter_warmup_period(df_monthly)

    df_monthly_feat = add_all_features(df_monthly.copy(), market_config)
    df_monthly_feat = df_monthly_feat[
        valid_history_mask.reindex(df_monthly_feat.index, fill_value=False)
    ]

    n_dropped = int((~valid_history_mask).sum())
    if n_dropped:
        logger.info(
            f"Warm-up : {n_dropped} lignes exclues (historique < {MIN_FEATURE_HISTORY} mois)."
        )
    ...
    monthly_dates = (
        df_monthly_feat.index.get_level_values("date").unique().sort_values()
    )
    ...
    for i, month_date in enumerate(monthly_dates[:-1]):
        ...
        selected_subset = (
            month_data[month_data["proba_upside"] >= PROBA_MIN]
            .sort_values("proba_upside", ascending=False)
            .head(MAX_STOCKS_SELECT)
        )
        ...
        if tickers:
            ...
            if not prices_subset.empty:
                ...
                probas_subset = selected_subset.loc[
                    prices_subset.columns, "proba_upside"
                ]
                weights, _ = get_optimal_weights(
                    prices_subset, probas_subset=probas_subset
                )
                allocation = {t: w for t, w in weights.items() if w > 1e-4}

        trading_days = daily_prices.index[
            (daily_prices.index >= month_date)
            & (daily_prices.index < monthly_dates[i + 1])
        ]
        period_df, portfolio_value, benchmark_value, drifted_allocation = (
            _simulate_period(
                allocation,
                drifted_allocation,
                trading_days,
